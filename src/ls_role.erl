...
set(Role, Attribute, Value) when
      is_binary(Role) ->
    send(libsnarl_msg:set(r(), Role, Attribute, Value)).
...
set(Role, Attributes) when
      is_binary(Role) ->
    send(libsnarl_msg:set(r(), Role, Attributes)).
...
-spec delete(Role::fifo:id()) ->
                          ...
                          ok.
delete(Role) ->
    send(libsnarl_msg:delete(r(), Role)).
...
grant(Role, Permission) ->
    send(libsnarl_msg:grant(r(), Role, Permission)).
...
revoke(Role, Permission) ->
    send(libsnarl_msg:revoke(r(), Role, Permission)).
...
revoke_prefix(Role, Prefix) ->
    send(libsnarl_msg:revoke_prefix(r(), Role, Prefix)).
